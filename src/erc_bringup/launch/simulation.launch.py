...
def generate_launch_description():
   ...
   # ── Bridge: gz-transport ↔ ROS 2 ──
    ...
    # Contact sensors
    # Keep every robot contact sensor on its own ROS topic. Mapping many
    # Gazebo Contacts publishers onto one ROS topic can make Python Contacts
    # subscriptions unreliable when real contacts arrive.
    _contact_topic_map = {}
    for _link in _re.findall(r'name="([^"]+)_contact" type="contact"', robot_description):
        _contact_topic_map[_link] = f'/contacts/{_link}'
